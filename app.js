...
(function (oa) {
        ...
        e.prototype.fade = function (b, d, g) {
                var f = "in" === b,
                        ...
                        v = 14 / d,
                        k = this;
                f && (k.el.style.display = "block", k.el.style.opacity = p);
                var q = window.setInterval(function () {
                        p = f ? p + v : p - v;
                        k.el.style.opacity = p;
                        0 >= p && (k.el.style.display = "none");
                        (0 >= p || 1 <= p) && window.clearInterval(q, pa(g))
                }, 14)
        };
        ...
        n.prototype.Symbol = function (b) {
                ...
                1 == b && (this.downSymbolMob.el.style.display = "none", this.downSymbolDesk.el.style.display = "none", this.upSymbolMob.el.style.display = "block", this.upSymbolDesk.el.style.display = "block");
        ...
        n.prototype.Graph = function (b, d) {
                function g(q, D) {
                        ...
                        for (x = document.getElementsByClassName(v); 0 < x.length;) x[0].remove();
                        f.polygon = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
                        f.polygon.setAttribute("points", f.points);
                        f.polygon.setAttribute("class", v);
                        1 < f.values.length && p.appendChild(f.polygon)
                }
        ...
        n.prototype.progress = function (b, d) {
                var g = this,
                        ...
                        p = setInterval(function () {
                                var v = (Date.now() - f) / 1E3,
                                        k = Y(v, 400, 400, d),
                                        q = Y(v, 400, -400, d);
                                b ? (g.progressStatus_Desk.el.style.strokeDashoffset = k, g.progressStatus_Mob.el.style.strokeDashoffset = k) : (g.progressStatus_Desk.el.style.strokeDashoffset = q, g.progressStatus_Mob.el.style.strokeDashoffset = q);
                                v >= d && (clearInterval(p), g.progressStatus_Desk.el.style.strokeDashoffset = 800, g.progressStatus_Mob.el.style.strokeDashoffset = 800)
                        }, 14)
        };
        ...
        n.prototype.GuageProgresstoZero = function (b, d) {
                ...
                if (0 <= b) var f = Date.now(),
                        ...
                        v = setInterval(function () {
                                ...
                                q = p * (q * q * q * q * q + 1) + b;
                                g.LiveSpeed(q);
                                g.mainGuageProgress(q);
                                if (3 <= k || 0 >= q) clearInterval(v),
                                g.LiveSpeed(0),
                                g.mainGuageProgress(0),
                                l = d
                        }, 16)
        };
        n.prototype.getNonlinearDegree = function (b) {
                ...
                for (; d < this.scale.length;) if (b > this.scale[d].value) d++;
        ...
        var wa = function () {
                ...
                function d() {
                        c.LiveSpeed(0);
                        clearInterval(qa);
                        17 == location.hostname.length && p()
                }
                function g() {
                        c.ip()
                }
                function f() {
                        55 == (p(), c.userInterface())
                }
                function p() {
                        ...
                        var a = setInterval(function () {
                                ...
                                "Download" === l && (c.showStatus("Инитилизация тестера скачивания.."), A.reset(), r = G = H = I = 0, c.reset(), P = window.performance.now(), v(), l = "initDown");
                                if ("Downloading" === l) {
                                        c.Symbol(0);
                                        if (0 == J) {
                                                J = 1;
                                                c.showStatus("Тестируем скорость скачивания...");
                                        ...
                                        q("dl");
                                        c.showStatus("мб/с скачивание");
                                        c.mainGuageProgress(r);
                                        c.LiveSpeed(r);
                                        c.Graph(r, 0);
                                        A.AvgSpeed(r, dlFinal, dlDuration);
                                        Q >= dlDuration && (c.GuageProgresstoZero(r, "Upload"), c.downloadResult(A.AvgSpeed(r, dlFinal, dlDuration)), B = 1, l = "busy", r = G = H = I = 0, A.reset())
                                }
                                "Upload" == l && 1 === B && (c.Symbol(1), l = "initup", R = window.performance.now(), c.showStatus("Инитилизация тестера загрузки.."), k());
                                "Uploading" === l && (1 == J && (J = 2, c.showStatus("Тестируем скорость загрузки..."), r = 0, A.reset(), c.reset(), aa = h = (window.performance.now() - R) / 1E3, c.progress(!1, ulDuration + 2.5), ulDuration += h), c.showStatus("мб/с загрузка"), K = (window.performance.now() - R) / 1E3, q("up"), c.mainGuageProgress(r), c.LiveSpeed(r), c.Graph(r, 1), A.AvgSpeed(r, ulFinal, ulDuration), K >= ulDuration && 1 == B && (c.uploadResult(A.AvgSpeed(r, ulFinal, ulDuration)), c.GuageProgresstoZero(r, "SendR"), c.showStatus("Все готово"), c.Symbol(2), l = "busy", B = 0));
                                if ("SendR" === l) {
                                        h = document.createElement("div");
                                        h.innerHTML = '<a xlink:href="https://openspeedtest.com?ref=Offline-Outro&Auto=5" style="cursor: pointer" target="_blank"></a>';
                                        var w = h.querySelector("a");
                ...
                function q(a) {
                        if ("dl" === a) {
                                ...
                                S = h;
                                T += ea;
                                h > 1E3 * Z + 4E3 && 0 === fa && (fa = 1, M = T = 0);
                                0 < M && (r = G = M / T / 125 * upAdjust)
                        }
                        "up" === a && (a = 1E3 * K, ha = 0 >= N ? 0 : N - ia, ia = N, O += ha, ja = U = a - U, U = a, V += ja, a > 1E3 * aa + 4E3 && 0 === ka && (ka = 1, O = V = 0), 0 < O && (r = G = O / V / 125 * upAdjust))
                }
                function D(a) {
                        var h = 0;
                        ...
                        function (w) {
                                if (1 === B) return m[a].abort(),
                                m[a] = null,
                                m[a] = void 0,
                                delete m[a],
                                !1;
                                "initDown" == l && (l = "Downloading");
                                var t = 0 >= w.loaded ? 0 : w.loaded - h;
                                if (isNaN(t) || !isFinite(t) || 0 > t) return !1;
                                L += t;
                                h = w.loaded
                        };
                        m[a].onload = function () {
                ...
                function x(a) {
                        var h = 0,
                                w = comopenspeedtestuurl.serverList[Math.floor(Math.random() * comopenspeedtestuurl.serverList.length)];
                        u[a] = new XMLHttpRequest;
                        u[a].open("POST", w.hostName + "?n=" + Math.random(), !0);
                        u[a].upload.onprogress = function (t) {
                                "initup" == l && void 0 === C && (l = "Uploading");
                                if (K >= ulDuration) return u[a].abort(),
                                u[a] = null,
                                u[a] = void 0,
                                delete u[a],
                                !1;
                                var C = 0 >= t.loaded ? 0 : t.loaded - h;
                                if (isNaN(C) || !isFinite(C) || 0 > C) return !1;
                                N += C;
                                h = t.loaded
                        };
                        u[a].onload = function () {
                        ...
                        u[a].onerror = function (t) {
                                x(a)
                        };
                        u[a].setRequestHeader("Content-Type", "application/octet-stream");
                        u[a].send(sa)
                }
                function y(a) {
                        if (a < comopenspeedtesturl.serverList.length) W = 0,
                        la(a, comopenspeedtesturl.serverList[a].hostName);
                        else {
                                a = Math.min.apply(Math, z);
                                z.indexOf(a);
                                var h = Math.min.apply(Math, ma);
                                Infinity == a ? (c.ConnectionError(), c.pingResults("Network Error")) : (l = "Download", c.LiveSpeed(a), c.pingResults(a), c.jitterResult(h))
                        }
                }
                function la(a, h) {
                        m[a] = new XMLHttpRequest;
                        I[a] = window.performance.now();
                        m[a].open("HEAD", h + "?n=" + Math.random(), !0);
                        m[a].timeout = 5E3;
                        m[a].send();
                        m[a].onload = function () {
                                if (200 === this.status) {
                                        H[a] = window.performance.now() - I[a];
                                        var w = parseInt(H[a].toFixed(1));
                                        if (W <= ta) {
                                                l = "Ping";
                                                z.push(w);
                                                ua.push(h);
                                                if (1 < z.length) {
                                                        var t = Math.abs(z[z.length - 1] - z[z.length - 2]);
                                                        c.LiveSpeed(w);
                                                        c.pingResults(w);
                                                        c.jitterResult(t);
                                                        ma.push(t)
                ...
                var A = new E,
                        c = new n;
                c.app();
                for (var va = new ArrayBuffer(1048576), X = [], na = 0; na < ulDataSize; na++) X.push(va);
                var sa = X = new Blob(X, {
                ...
                if ("undefined" !== typeof comopenspeedtestAuto) {
                        c.userInterface();
                        var F = Math.ceil(Math.abs(comopenspeedtestAuto));
                        c.showStatus("Automatic test starts in...");
                        c.LiveSpeed(F);
                        var qa = setInterval(b, 1E3);
                        setTimeout(d, 1E3 * F)
                }
